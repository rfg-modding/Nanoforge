...
public partial class Viewport3D : UserControl
{
    ...
    public Viewport3D()
    {
        ...
        Vector2D<int> viewportSize = MakeViewportSize((int)ViewportImage.Width, (int)ViewportImage.Height);
        CreateRendererOutputTexture(viewportSize);
    ...
    private void CreateRendererOutputTexture(Vector2D<int> size)
    {
        _rendererOutput = new WriteableBitmap(new PixelSize(size.X, size.Y), new Vector(96, 96), PixelFormat.Bgra8888);
        ViewportImage.Source = _rendererOutput;
    }
    ...
    private void RenderFrame()
    {
        ...
        if (Scene == null)
        {
            LoadingStatus = "Scene not set on Viewport3D control!";
            return;
        }
        ...
        SceneFrameUpdateParams updateParams = new((float)deltaTime.TotalSeconds, (float)totalTime.TotalSeconds, _leftMouseButtonDown, _rightMouseButtonDown, _mousePosition,
            _mousePositionDelta, _mouseOverViewport);
    ...
    {
        Vector2D<int> newSize = MakeViewportSize((int)e.NewSize.Width, (int)e.NewSize.Height);
        CreateRendererOutputTexture(newSize);
        Scene?.ViewportResize(newSize);
    }

    //This is used to calculate the new viewport size to make sure the vulkan code and avalonia code have the same exact size. Had a crash from them being slightly different due to doing different type casts.
    private Vector2D<int> MakeViewportSize(int width, int height)
    {
