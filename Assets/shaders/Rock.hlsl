...
//TODO: Use includes to reduce the amount of duplicate shader code. Will need to update shader hot reload code to track when included files change as well.
...
VS_OUTPUT VS(float4 inPos : POSITION, float4 inNormal : NORMAL, float4 inTangent : TANGENT, int2 inUv : TEXCOORD)
{
    ...
    output.Tangent = mul(inTangent, Rotation);
